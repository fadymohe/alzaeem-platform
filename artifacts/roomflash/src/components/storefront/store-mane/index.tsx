...
export function StoreManeTheme({
  ...
  const announcement = customization?.announcementText || 'صالون وعناية MANE — مستحضرات تجميل أصلية 100% مع شحن سريع لجميع محافظات العراق';
  ...
  return (
    ...
    >
      ...
      {/* 2. Deep Burgundy Header (Matching Screenshot 2 Mane) */}
      <header
      ...
      >
        <div className="max-w-7xl mx-auto px-4 md:px-8 py-3.5 flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            {logoUrl ? (
              <img src={logoUrl} alt={storeName} className="h-10 w-auto max-w-[140px] object-contain rounded-xl border border-white/20" />
            ) : (
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h1 className="font-black text-2xl text-white tracking-tight leading-none lowercase">
                    {storeName === 'متجر الزعيم' ? 'mane' : storeName}
                  </h1>
          ...
          {/* Navigation Links */}
          <nav className="hidden lg:flex items-center gap-6 text-xs font-black text-pink-100">
            {['الرئيسية', 'العناية', 'الشعر', 'سيرومات', 'مرطبات', 'كولاجين', 'الجديد'].map((item) => (
              <button
                key={item}
                type="button"
              ...
              >
                {item}
                {activeTab === item && (
                  <span className="absolute -bottom-2 right-0 left-0 h-0.5 bg-pink-300 rounded-full" />
                )}
              </button>
            ))}
          </nav>
          ...
          <div className="flex items-center gap-3">
            <div className="relative hidden sm:block w-56">
              <Search className={`absolute ${isEn ? 'left-3.5' : 'right-3.5'} top-2.5 size-4 text-pink-200`} />
              <input
                type="text"
                value={searchQuery}
                onChange={(e) => onSearchChange(e.target.value)}
                placeholder={isEn ? 'Search serums...' : 'ابحثي عن مستحضر...'}
                className={`w-full h-9 ${isEn ? 'pl-10 pr-4' : 'pr-10 pl-4'} rounded-full border border-white/20 bg-white/10 text-xs text-white placeholder:text-pink-200/60 focus:outline-none focus:bg-white/20`}
              />
            </div>
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-full bg-white text-[#6b0f24] font-black text-xs shadow-md transition-all hover:scale-105"
            >
        ...
        <div className={`grid ${getColsClass()} gap-6`}>
          {filteredProducts.map((p) => (
            ...
            >
              ...
              <div className="p-5 border-t border-pink-50 bg-[#fdfafb] flex items-center justify-between">
                <div>
                ...
                {enableQuick && (
                  <button
                    ...
                    className="px-4 py-2 rounded-xl text-xs font-black text-white shadow-md flex items-center gap-1 transition-all hover:scale-105"
                    style={{ backgroundColor: brandColor }}
                  >
