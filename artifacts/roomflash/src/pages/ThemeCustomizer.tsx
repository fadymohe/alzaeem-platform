...
export function ThemeCustomizerPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0d131f] text-slate-100 font-sans antialiased flex flex-col" dir="rtl">
      ...
      {/* 1. Top Offer/Alert Banner (Matching Screenshot 1 Baseet Customizer) */}
      <div className="bg-gradient-to-r from-amber-600 to-amber-700 text-slate-950 font-black text-xs py-2 px-4 flex flex-wrap items-center justify-between gap-3 shadow-md border-b border-amber-500/30">
        <div className="flex items-center gap-3">
          <span className="px-2 py-0.5 rounded bg-slate-950 text-amber-400 font-mono text-[11px] font-black">
            06 : 23 : 59 : 11
          </span>
          <span>فعّل متجرك الآن — باقة الزعيم الاحترافية مع دومين محجوز وشحن مباشر لكافة المحافظات</span>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-[11px] font-bold text-slate-900 hidden sm:inline">0/5 متبقي 5 طلبات مجانية</span>
          <button
            type="button"
            onClick={() => setLocation('/subscriptions')}
            className="px-3 py-1 bg-slate-950 text-amber-400 hover:bg-slate-900 rounded-lg text-xs font-black shadow transition-all"
          >
            فعّل العرض الآن
          </button>
        </div>
      ...
      {/* 2. Customizer Navigation Tabs Header */}
