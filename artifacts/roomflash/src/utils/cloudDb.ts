...
/**
 * Iraqi Phone Number Normalization Helper
 */
...
/**
 * Persist user registration to Neon PostgreSQL and local cache
 */
export async function saveCloudUser(user: {
  email: string;
  phone: string;
  name?: string;
  governorate?: string;
  ...
  try {
    ...
    const normPhone = normalizeIraqiPhone(user.phone || '');
    const nameEsc = (user.name || normEmail.split('@')[0] || '').replace(/'/g, "''");
    const govEsc = (user.governorate || 'بغداد').replace(/'/g, "''");
    const storeEsc = (user.storeName || '').replace(/'/g, "''");
    const subEsc = (user.subdomain || '').replace(/'/g, "''");

    // 1. Save to local storage cache
    try {
      const usersList: any[] = JSON.parse(localStorage.getItem('zaeem_registered_users') || '[]');
      const filtered = usersList.filter((u: any) => u.email !== normEmail && u.phone !== normPhone);
      filtered.unshift({
        email: normEmail,
        phone: normPhone,
        name: user.name,
      ...
      localStorage.setItem('zaeem_registered_users', JSON.stringify(filtered));
    } catch {}

    // 2. Save via Backend API endpoint
    try {
    ...
    const ensureTableSql = `
      CREATE TABLE IF NOT EXISTS za3em_users (
        id SERIAL PRIMARY KEY,
        email TEXT UNIQUE,
        phone TEXT,
        name TEXT,
    ...
    `;
    await executeSql(ensureTableSql);

    const insertSql = `
      INSERT INTO za3em_users (email, phone, name, governorate, store_name, subdomain)
      VALUES ('${normEmail}', '${normPhone}', '${nameEsc}', '${govEsc}', '${storeEsc}', '${subEsc}')
    ...
    console.warn('[CloudDb] Error saving cloud user:', err);
    return false;
  }
}

/**
 * Check if a phone number is already registered in the system (Sign-Up check)
 */
export async function checkCloudPhoneExists(phone: string): Promise<{
  ...
  const normPhone = normalizeIraqiPhone(phone);
  ...
  try {
    ...
    if (rawUser) {
      ...
      if (u?.phone && normalizeIraqiPhone(u.phone) === normPhone) {
        return { exists: true, email: u.email, name: u.name, storeName: u.storeName };
      }
    }
    const rawUsersList = localStorage.getItem('zaeem_registered_users');
    if (rawUsersList) {
      const list = JSON.parse(rawUsersList);
      const found = list.find((u: any) => u.phone && normalizeIraqiPhone(u.phone) === normPhone);
      if (found) {
        return { exists: true, email: found.email, name: found.name, storeName: found.storeName };
      }
    }
  } catch {}

  // 2. Check Backend Vite API
  try {
    ...
    if (res.ok) {
      ...
      if (data?.exists) {
        return { exists: true, email: data.email, name: data.name, storeName: data.storeName };
      }
    }
  } catch {}

  // 3. Check Neon PostgreSQL database
  try {
    const query = `
      SELECT email, phone, name, store_name, subdomain FROM za3em_users 
    ...
    if (result && Array.isArray(result.rows) && result.rows.length > 0) {
      ...
      return { exists: true, email: r.email, name: r.name, storeName: r.store_name };
    }
  } catch {}
...
/**
 * Lookup user account by phone number or email for Sign-In
 */
export async function lookupUserByPhone(phoneOrEmail: string): Promise<{
  found: boolean;
  email?: string;
  name?: string;
  phone?: string;
  ...
  if (input.includes('@')) {
    return { found: true, email: input.toLowerCase() };
  }

  const normPhone = normalizeIraqiPhone(input);
  if (!normPhone || normPhone.length < 10) {
    return { found: false };
  }
